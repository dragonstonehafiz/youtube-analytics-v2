...
- `try_begin_sync(stage_keys)` atomically rejects an existing reservation or sets `active=true`, clears `stop_requested`, and seeds the selected keys as pending in canonical order. The manual trigger passes its validated stage list; startup passes `full_incremental_plan()`'s list. A successful reservation replaces the prior plan's stage results.
...
When a checkpoint raises `SyncCancelled`, `_run_stage()` persists the active stage via `database.cancel_sync_run()` with its partial counters, logs it at `INFO` (not `ERROR`), and re-raises. `_run_tracked_stage()` records the affected live stage as cancelled. On the serial path this unwinds to `execute_plan()`, which releases the reservation; on the Analytics API path the worker marks itself cancelled and stops its own queue while its sibling can finish. During finalization, every selected stage still pending after an accepted stop is also marked cancelled, so the stopped plan retains an explicit outcome even when the stop landed between stages. Stages that already completed keep their success rows; stages after the cancelled one never start and get no row.
...
- `_run_stage()` (`sync/orchestration.py`) always re-raises the underlying exception after recording failure via `fail_sync_run()`. On the serial pre-analytics path, the stage status is marked failed and the exception propagates to the caller. On the Analytics API path, `_run_tracked_stage()` and `_run_analytics_worker()` catch it; see [Analytics API worker split](#analytics-api-worker-split). It also logs the stage start, completion, and failure — see [Sync logging](#sync-logging).
