...
- The worklist is `database.get_all_video_ids()`, the same catalog every other per-video stage reads — no separate discovery call.
...
| Video processed | the per-video loops in `sync_video_analytics()`/`sync_video_traffic_sources()`/`sync_search_related_insights()` | ordinal/total, `video_id`, rows fetched for that video (search insights also logs the month count and the actual weekly-chunked API call count for that video), title |
| Video skipped | the two `continue` branches in each analytics stage, and `sync_search_related_insights()`'s `scope="year"`/`"all"` no-publish-date branch | ordinal/total, `video_id`, reason (`no_publish_date` or `empty_range`), title |
