...
- `backend/database/` (sync-run helpers only: `create_sync_run`/`complete_sync_run`/`fail_sync_run`/`get_sync_runs`/`mark_incomplete_sync_runs`/`get_last_successful_run_completed_at` in `sync_runs.py`, `get_last_analytics_date` in `analytics.py`, `get_last_traffic_source_date` in `traffic_sources.py`, `get_last_fx_rate` in `fx_rates.py`, `get_owned_video_ids`/`get_all_video_ids`/`upsert_own_video`/`upsert_related_video` in `videos.py`, `upsert_search_terms`/`get_last_search_terms_month` in `search_terms.py`, `upsert_related_videos`/`get_last_related_videos_month` in `related_videos.py`)
...
```
playlists → videos → comments → pruning → video_analytics → video_traffic_sources → search_insights → related_video_insights → fx_rates
```
...
`search_insights` and `related_video_insights` share no dependency with `video_traffic_sources`, or with each other — canonical order only controls their placement when more than one happens to be selected in the same plan; selecting or deselecting any one of them never affects the others. See [Search insights synchronization](#search-insights-synchronization) and [Related video insights synchronization](#related-video-insights-synchronization).
...
`sync_search_insights(scope, year, counts)` (`sync/stages.py`) fetches and upserts monthly Search-source terms for every owned video, month-by-month but fetched in weekly sub-chunks per month (see below — this raises how much of a month's real search traffic gets captured under the API's per-request 25-row cap). It is period-aware, like `video_analytics`/`video_traffic_sources`: `scope="incremental"` (the default, "New data only") resumes from each video's own last stored month through yesterday, backfilling from `publish_date` if it has no stored terms at all yet — matching what "New data only" means for `video_analytics`/`video_traffic_sources` (`get_last_analytics_date`/`get_last_traffic_source_date` + `_incremental_lookback_start`), not just "has any data at all, so skip straight to a fixed narrow window." `scope="year"`/`"all"` refresh a wider range, clamped per video to its own publish date.

- `scope="incremental"`, video has a `published_at`: `start = publish_date` if `database.get_last_search_terms_month(video_id)` returns `None` (never synced), else `max(date(last_month, day=1), publish_date)` — the last stored month is re-checked (the monthly equivalent of the daily lookback re-checking recent days that may not have settled), then `monthly_windows_for_range(start, yesterday)` (below) generates every month from there through now. A video whose last stored month is already last month collapses to exactly the current+previous window, same as the original fixed behavior; a video whose last stored month is much older (e.g. an interrupted backfill, or a video that has sat unsynced for a while) resumes filling every month in between rather than being wrongly treated as fully caught up just because it has *some* stored data.
...
- The worklist is `database.get_owned_video_ids()`, the same owned-only catalog `video_analytics`/`video_traffic_sources`/`related_video_insights` read — no separate discovery call.
- Each `MonthlyWindow` (any scope) is fetched as multiple weekly requests, not one per month: `monthly_insights.weekly_sub_windows(window)` (`sync/monthly_insights.py`) splits it into consecutive 7-day `(start_date, end_date)` chunks (the last chunk may be shorter), and `youtube.fetch_video_search_terms(video_id, start_date, end_date)` (`youtube/analytics_api.py`) is called once per chunk — `dimensions=insightTrafficSourceDetail`, `metrics=views`, `filters=video==<id>;insightTrafficSourceType==YT_SEARCH`, `sort=-views`, `maxResults=25`, `startIndex` omitted entirely. The Search Analytics detail report hard-caps each request's result set at 25 rows total with **no pagination past it** — verified live against the production API (`search-insights-api-findings.md`): `maxResults>25` and any `startIndex` reaching row 26 both return HTTP 500, not a normal next page. Since the cap is per request rather than per month, narrowing each request to a week (instead of the whole month) raises how many of a month's real terms get captured before the rest is silently dropped. Each chunk's returned term views are combined in memory per `search_term` (summed across chunks within the same month, not overwritten) before one `database.upsert_search_terms(video_id, window.month, combined_terms)` call per month (see `database.md`) — storage stays month-granular; only the fetch is weekly. `counts.rows_fetched` accumulates the raw row count across all chunks (including zero-view rows storage drops); `counts.rows_written` accumulates each month's single upsert count. A `scope="all"`/`"year"` run issues these weekly-chunked calls for every calendar month in range per video — full history is more calls of the same shape, not a heavier kind of request.
...
`sync_related_video_insights(scope, year, counts)` (`sync/stages.py`) fetches and upserts monthly Related Video referrers for every owned video, then resolves metadata for newly encountered referrer IDs. It shares `sync_search_insights`'s period-aware shape (`scope="incremental"` resumes from each video's own last stored month via `database.get_last_related_videos_month(video_id)`, backfilling from `publish_date` if unsynced, falling back to the fixed current+previous window for a video with no `published_at`; `scope="year"`/`"all"` refresh a wider range clamped per video to its own publish date), fetched in the same weekly sub-chunks per month for the same 25-row-per-request reason (see [Search insights synchronization](#search-insights-synchronization) and `search-insights-api-findings.md`). This metadata-resolution logic lives only here, not in `sync_search_insights`.

- The worklist is `database.get_owned_video_ids()`, captured once at stage start — a referrer resolved into `videos` during this same run can never become a target within the same run.
- Each chunk calls `youtube.fetch_video_related_videos(video_id, start_date, end_date)` (`youtube/analytics_api.py`) — `dimensions=insightTrafficSourceDetail`, `metrics=views`, `filters=video==<id>;insightTrafficSourceType==RELATED_VIDEO`, `sort=-views`, `maxResults=25`, `startIndex` omitted entirely; parsing (header lookup, the 25-row cap check, malformed-row rejection, positive-views-only filtering) is a shared helper with Search's own parsing, since the two response shapes are identical apart from the traffic-source-type filter and what the detail value represents. Each chunk's referrer views are combined in memory per `referrer_video_id` (summed across chunks within the same month) before one `database.upsert_related_videos(video_id, window.month, combined_referrers)` call per month.
...
- After every video/month has been fetched, `_resolve_related_video_metadata(newly_encountered_ids, counts)` resolves referrer IDs not already present in `videos` (via `database.get_all_video_ids()`, deliberately unfiltered by ownership — checking "known at all," not "owned"; see `database.md`). Unknown IDs are sorted for determinism and resolved in batches of at most 50 via `youtube.fetch_videos()`, then classified against the authenticated channel ID from `youtube.fetch_channel_identity()` and written via `database.upsert_related_video(video, own=matches_channel)`. A batch lookup failure is logged at `WARNING` and skipped — it never fails the stage or discards Related rows already stored. An ID omitted from its batch's response simply gets no video row and is left to be retried on a future run.
...
| Video processed | the per-video loops in `sync_video_analytics()`/`sync_video_traffic_sources()`/`sync_search_insights()`/`sync_related_video_insights()` | ordinal/total, `video_id`, rows fetched for that video (search and related insights also log the month count and the actual weekly-chunked API call count for that video), title |
