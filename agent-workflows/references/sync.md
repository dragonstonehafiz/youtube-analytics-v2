...
- If `start > range_end`, the video is skipped entirely via `continue` — **zero API calls** for that video. This defensive branch now only fires for inconsistent checkpoint data (see above); a video published after the stage's effective range end was already excluded from the worklist and never reaches this line.
...
- Same prefiltered-worklist behavior as analytics — see [Prefiltered worklist (effective range end)](#prefiltered-worklist-effective-range-end).
