...
`sync_search_related_insights(scope, year, counts)` (`sync/stages.py`) fetches and upserts monthly Search-source terms for every stored video. It is period-aware, like `video_analytics`/`video_traffic_sources`: `scope="incremental"` (the default, "New data only") refreshes the current and previous calendar months for a video that already has stored terms, or backfills from that video's own publish date on its first run — matching what "New data only" means for `video_analytics`/`video_traffic_sources` (a never-synced video backfills from `publish_date`; only a previously-synced one gets the narrow catch-up window). `scope="year"`/`"all"` refresh a wider range, clamped per video to its own publish date.

- `scope="incremental"`, video already has stored terms (`database.get_last_search_terms_month(video_id)` returns non-`None`): `monthly_insights.monthly_search_windows(today)` (`sync/monthly_insights.py`) is a pure calendar function, captured once via `date.today()` at the start of the stage (before the per-video loop) so a midnight rollover mid-run cannot change this part of the worklist. It returns the previous month's full window (1st through last day) first, then the current month's window (1st through yesterday) — omitted entirely when `today` is the 1st, since that window would otherwise be empty. Real calendar arithmetic, not a 30-day approximation, so it handles leap February and January-to-December rollover correctly. The previous-month refresh stands in for the 7-day daily lookback `video_analytics`/`video_traffic_sources` use to catch data unsettled at first fetch — there is no daily granularity here to look back within, so the whole adjacent month is re-checked instead.
- `scope="incremental"`, video has never had any stored terms (`get_last_search_terms_month` returns `None`) and has a `published_at`: backfills via `monthly_windows_for_range(publish_date, yesterday)` (below) instead of the fixed two-month window — the same "first sync starts from publish date" behavior `video_analytics`/`video_traffic_sources` already have. A video with no `published_at` falls back to the fixed current+previous window regardless of prior data, since there is no date to backfill from.
...
- Per video/window (any scope), `youtube.fetch_video_search_terms(video_id, start_date, end_date, title=title)` (`youtube/analytics_api.py`) issues exactly one `reports.query` call — `dimensions=insightTrafficSourceDetail`, `metrics=views`, `filters=video==<id>;insightTrafficSourceType==YT_SEARCH`, `sort=-views`, `maxResults=25`, `startIndex` omitted entirely — then `database.upsert_search_terms(video_id, window.month, result.terms)` persists it (see `database.md`). `counts.rows_fetched` accumulates the raw row count (including zero-view rows storage drops); `counts.rows_written` accumulates the upsert's own count. A `scope="all"`/`"year"` run issues one such call per calendar month in range per video, not a different or heavier kind of request — full history is simply more of the same call.
...
| Search insights stage summary | `sync/stages.py::sync_search_related_insights()`, once at the end of the stage | scope, year, video count, total rows fetched/written |
