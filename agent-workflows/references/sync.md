...
| Video processed | the per-video loops in `sync_video_analytics()`/`sync_video_traffic_sources()`/`sync_search_related_insights()` | ordinal/total, `video_id`, rows fetched for that video (search insights also logs the month count for that video), title |
