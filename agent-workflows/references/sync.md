...
- `get_sync_status()` (`sync/status.py`) exposes `{state, message}` guarded by a module-level `threading.Lock`, where `state` is one of `idle | running | stopping | success | failed | cancelled` — safe to poll from any thread. All mutation of that shared state is centralized in `sync/status.py` behind `try_begin_sync()`, `update_sync_progress()`, `request_stop()`, `raise_if_stopping()`, `complete_sync()`, `fail_sync()`, `cancel_sync()`, and `reset_sync_status()`.
- `try_begin_sync(message="")` (`sync/status.py`) sets `state="running"` **and** the initial message under one lock acquisition, so a status poll can never observe the running state still carrying the previous run's terminal message. It rejects reservation while `state` is `running` **or** `stopping`, so a stop request blocks a new sync from starting until the stopping worker actually exits. It is the single reservation primitive: the manual route calls it before responding, and `run_plan()` calls it for the startup sync. Whichever loses the race gets `False` — the route turns that into `409`, the startup sync silently declines. A successful reservation replaces any retained terminal result (`success`/`failed`/`cancelled`) from the previous run.
- `update_sync_progress(message)` is a no-op unless `state="running"`, so a stray call cannot fabricate a running state and cannot overwrite the `stopping` message once cancellation has been requested.
...
- `sync/orchestration.py`'s `execute_plan()` checks before each stage's row is created, and once more before the final `complete_sync()` call.
- `sync/stages.py` checks between videos/playlists/comments/months/batches in every stage's own loop, and immediately before the pruning delete and each FX-rate day.
...
When a checkpoint raises `SyncCancelled`, `_run_stage()` (`sync/orchestration.py`) catches it ahead of the generic exception handler, persists the active stage via `database.cancel_sync_run()` with its partial counters, logs it at `INFO` (not `ERROR`), and re-raises so `execute_plan()`'s own handler records the plan-level `cancelled` terminal state. Stages that already completed keep their `success` rows; stages after the cancelled one never start and get no row — identical to the fail-fast behavior on a genuine exception, except the terminal state and log severity differ.
