...
**`Sync.tsx`** — Manual sync form; the only place a sync can be triggered. A `.data-table.sync-table` with three columns (Include \| Stage \| Period) renders one row per stage from a module-level `STAGE_ROWS` constant in canonical order. All five checkboxes start checked and both period selectors start at "New data only". `videos`/`playlists`/`fx_rates` show a plain "Not applicable" in the Period column; `video_analytics`/`video_traffic_sources` each get an independent `StagePeriodSelect` (New data only \| Full history \| then each year from the current year back down to `/meta/date-range`'s `earliest_year`). Unchecking a stage disables its selector but **keeps** the selection, so rechecking restores it. State is local component state, not URL params — the plan is an ephemeral action, not a shareable filter (contrast [URL and filter state](#url-and-filter-state)). Does not reuse `PeriodSelect.tsx`, whose date-range semantics don't match sync scopes. Polls `/sync/status` every 5s; an active sync disables every control, and the live progress message renders in the navbar's `SyncStatus` component (not on this page — there is no `.sync-banner` here). A `.sync-status-banner` block on this page appears only when status is unknown (unreached endpoint or not yet loaded), not during a normal active sync. The full-width "Sync selected" button is disabled when nothing is selected, while submitting, or while a sync is active — the backend still rejects empty and racing submissions authoritatively. API rejections render in a `.sync-error` block rather than being swallowed.
