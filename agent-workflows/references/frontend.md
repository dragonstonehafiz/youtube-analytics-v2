...
export interface SyncState { is_syncing: boolean; message: string }
...
- `triggerSync(plan)` is the one wrapper that does **not** follow the file's `fetch(...).then(r => r.json())` shape. It POSTs a JSON `SyncPlan` body with an explicit `Content-Type`, checks `response.ok`, and rejects with an `Error` built by `syncErrorMessage(response)` — which reads FastAPI's `detail` field as either a plain string (400/409) or the first entry's `msg` from a 422 validation array, falling back to the status code. Every other wrapper in the file resolves regardless of status, so a failed request would otherwise surface as a successful parse of an error body.
...
- `getTopVideosByViews(sortBy, startDate?, endDate?, contentType?, privacyStatus?)` and `getPlaylistTopVideosByViews(id, sortBy, startDate?, endDate?, contentType?, privacyStatus?)` take a required `TopVideoSortBy` (`'views' | 'watch_time'`) as their first optional-adjacent argument, positioned before the date/filter params. There is no default — every repository call site must pass one explicitly, so `tsc` catches an omitted sort rather than it silently defaulting. All 8 current call sites are accounted for: 6 pass `'views'` (`Home.tsx` ×2, `Analytics.tsx`/`PlaylistAnalytics.tsx` seven-day sidebar effects ×2 each), 2 pass `'watch_time'` (the main `TopVideosList`-feeding effect in `Analytics.tsx` and `PlaylistAnalytics.tsx`).
...
| `SyncStatus.tsx` | Display only — polls `/sync/status` every 5s and renders nothing else. Syncing: animated blue dot + message (`.sync-status-dot`/`.sync-status-message`). Idle: a compact neutral "Not syncing" (`.sync-status-idle`); no last-synced text. Returns `null` until the first poll resolves. It holds **no** manual sync controls — triggering a sync lives on the `/sync` page. The `sync-pulse` keyframes it animates with are defined once in `index.css`; `Sync.css` does not use them. Idle rendering ignores `status.message` — a failed sync's message is fetched but never shown once `is_syncing` goes false (see the note under `Sync.tsx` below). |
...
- `Sync.tsx` is deliberately **not** query-driven: its stage/period selections are a one-shot action, so they live in component state and are not shareable or restorable from the URL.
