...
- `get_sync_runs(page, page_size)` (`database/sync_runs.py`) returns `tuple[list[dict], int]` — one page of **sync batches** plus the distinct-batch total. A batch is one `batch_id`, the ID `execute_plan()` generates once per submitted plan and shares across every stage that starts, so paging counts submitted syncs rather than stage rows. Each group is `{batch_id, started_at, run_count, rows_fetched, rows_written, rows_deleted, runs}`. It takes no filters, so no statement has a `WHERE` clause other than the batch-ID lookup. Three statements run on one connection:
