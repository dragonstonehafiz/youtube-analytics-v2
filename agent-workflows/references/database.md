...
- **Title filter**: every analytics/traffic-source/published-videos helper (`get_aggregated_analytics`, `get_top_videos_by_views`, `get_playlist_top_videos_by_views`, `get_playlist_aggregated_analytics` in `database/analytics.py`; `get_aggregated_traffic_sources`, `get_playlist_aggregated_traffic_sources`, `get_top_videos_by_traffic_source`, `get_playlist_top_videos_by_traffic_source` in `database/traffic_sources.py`; `get_videos_published` in `database/videos.py`) accepts an optional `title` parameter and, when non-empty, appends `v.title LIKE ?` bound to `f"%{title}%"` — the same case-insensitive partial-match semantics `get_all_videos()`/`get_video_stats()`/`get_playlist_video_stats()` already used, combined with any other supplied condition via `AND`. On playlist-scoped helpers this is additive to, not a replacement for, the existing playlist-membership condition — a title match outside the playlist still yields no row for that helper's playlist. An omitted or empty title leaves the query unchanged from before this filter existed.
