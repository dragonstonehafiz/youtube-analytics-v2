...
- `get_top_videos_by_views()` and `get_playlist_top_videos_by_views()` (`database.py:833-980`) validate `sort_by` against `_TOP_VIDEO_SORT_ORDER_BY`, a mapping from public sort value to a full `ORDER BY` clause (aggregate plus deterministic tie-breakers), not a bare column name:
